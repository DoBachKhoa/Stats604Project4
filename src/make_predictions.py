...
from src.pipeline import BaseMeanByDayPipeline, Edison, Ampere
...
if __name__ == '__main__':
    ...
    for zone in ZONES:
        if prediction_model == 'Ampere':
            pipeline = Ampere(param_dir='pca_params/global_params_2025', correction_days=slide_week_day(-1, 1, daystart=PRED_WEEK_START), \
        ...
        elif prediction_model == 'Edison':
            pipeline = Edison(param_dir='pca_params/global_params_2025', \
                              zone=zone, year=datetime.now().year, train_year=4, num_PC=5, pca_input_dir='data/data_weather_hourly_processed')
            pipeline.train_model()
            weather = hourly_open_meteo(zone, WEATHER_FEATURES_HOURLY_COUNTS, tomorrow)
            d, w = weather.iloc[0]['day_of_week'], weather.iloc[0]['relative_week']
